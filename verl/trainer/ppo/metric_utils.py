...
def _compute_response_info(batch: DataProto) -> Dict[str, Any]:
    ...
    """
    response_shape = batch.batch["responses"].shape[-1]

    prompt_mask = batch.batch["attention_mask"][:, :-response_shape]
    response_mask = batch.batch["attention_mask"][:, -response_shape:]

    # Takes care of the multi-turn scenario
    ...
    if loss_mask is None:
        ...
        response_length = loss_mask.sum(-1).float()
...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    # Count total prompts for progress logging
    total_prompts = sum(len(prompts) for prompts in data_src2prompt2var2vals.values())
    processed_prompts = 0
    print(f"[process_validation_metrics] Starting bootstrap for {total_prompts} prompts across {len(data_src2prompt2var2vals)} data sources")

    # Calculate metrics for each group
    data_src2prompt2var2metric = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
    for ds_idx, (data_source, prompt2var2vals) in enumerate(data_src2prompt2var2vals.items()):
        ds_num_prompts = len(prompt2var2vals)
        print(f"[process_validation_metrics] Processing data source {ds_idx+1}/{len(data_src2prompt2var2vals)}: {data_source} ({ds_num_prompts} prompts)")
        for prompt_idx, (prompt, var2vals) in enumerate(prompt2var2vals.items()):
            for var_name, var_vals in var2vals.items():
                ...
                if var_vals[0] is None:
                    continue
                # Also check if any value is None (can happen with extracted_answer)
            ...
            # Progress logging every 100 prompts or at the end
            processed_prompts += 1
            if processed_prompts % 100 == 0 or processed_prompts == total_prompts:
                print(f"[process_validation_metrics] Processed {processed_prompts}/{total_prompts} prompts ({100*processed_prompts/total_prompts:.1f}%)")

    print(f"[process_validation_metrics] Bootstrap complete, aggregating metrics...")
