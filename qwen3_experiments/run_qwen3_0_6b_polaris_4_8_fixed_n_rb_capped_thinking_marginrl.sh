...
export MAXRL_ADVANTAGE_ESTIMATOR=fixed_n_rb_capped_thinking_cost_aware_marginrl
...
export MAXRL_POST_THINK_PRE_BOX_TOKEN_LIMIT=${MAXRL_POST_THINK_PRE_BOX_TOKEN_LIMIT:-512}
export MAXRL_PROJECT_NAME=${MAXRL_PROJECT_NAME:-Qwen3_MaxRL_Experiments}
export MAXRL_EXPERIMENT_NAME=${MAXRL_EXPERIMENT_NAME:-fixed_n_rb_capped_thinking_cost_aware_marginrl_Qwen3-0.6B_polaris_4_8_cap4_postthink512_2epochs}

# Cost counts only tokens strictly between <think> and </think>. Reward is one
# only when MathVerify is correct and the strictly open </think>...\boxed{
# span contains fewer than 512 tokens. Missing structural markers receive zero
# reward; a missing </think> is charged through the observed response end.
exec "${SCRIPT_DIR}/run_qwen3_1_7b_math12k.sh" \
    "+reward_model.reward_kwargs.post_think_pre_box_token_limit=${MAXRL_POST_THINK_PRE_BOX_TOKEN_LIMIT}" \
    "$@"
